			...
			spec?: {
				...
				containers!: [...{
					...
					lifecycle?: {
						...
						// period (unless delayed by finalizers). Other management of the container
					...
					// first client attaches to stdin, and then remains open and accepts data until
				...
				// Once this field is set, the kubelet for this node becomes responsible for the
				// lifecycle of this pod.
				...
				// the RuntimeClass admission controller is enabled, overhead must not be set in
				// Pod create requests.
				// The RuntimeClass admission controller will reject Pod create requests which
				// have the overhead already
				...
				volumes?: [...{
					...
					// A failure to resolve or pull the image during pod startup will block
					...
					// container runtime implementation on a host machine and at minimum must
					// include all valid types supported by the container image field.
					// The OCI object gets mounted in a single directory
					// (spec.containers[*].volumeMounts.mountPath) by merging the manifest layers
					// in the same way as for container images.
