...
#Cluster: {
	...
	spec!: {
		...
		affinity?: {
			...
			// added to the ones generated
			// by the operator if EnablePodAntiAffinity is set to true (default) or to be
			// used exclusively if set to false.
			...
			// between cluster instance has to be
			// considered a strong requirement during scheduling or not. Allowed values are:
			// "preferred" (default if empty) or
			// "required". Setting it to "required", could lead to instances remaining
			// pending until new kubernetes nodes are
			// added if all the existing nodes don't match the required pod anti-affinity rule.
			// More info:
			// https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/#inter-pod-affinity-and-anti-affinity
			podAntiAffinityType?: string

			// Tolerations is a list of Tolerations that should be set for all the pods, in
			// order to allow them to run
			// on tainted nodes.
		...
		backup?: {
			...
			volumeSnapshot?: {
				...
				onlineConfiguration?: {
					...
					// Otherwise, WAL required to make the backup consistent might be missing and
					// make the backup useless.
					// By default, or when this parameter is true, pg_backup_stop will wait for WAL
					// to be archived when archiving is
					// enabled.
			...
			volumeClaimTemplate?: {
				...
				spec!: {
					...
					dataSourceRef?: {
						...
						// gateway.networking.k8s.io/ReferenceGrant object is required in the referent
						// namespace to allow that namespace's owner to accept the reference. See the
		...
		// The instances' log level, one of the following values: error, warning, info
		// (default), debug, trace
		logLevel?: "error" | "warning" | "info" | "debug" | "trace"

		// The configuration that is used by the portions of PostgreSQL that are managed
				...
				additional?: [...{
					...
					serviceTemplate!: {
						...
						spec?: {
							...
							// balancer implementation is used, today this is typically done through the
							// cloud provider integration,
							// but should apply for any default implementation. If set, it is assumed that a load balancer
							// implementation is watching for Services with a matching class. Any default load balancer
							// implementation (e.g. cloud providers) should ignore Services that set this field.
							// This field can only be set when creating or updating a Service to type 'LoadBalancer'.
							// Once set, it can not be changed. This field will be wiped when a service is
							// updated to a non 'LoadBalancer' type.
							...
							ports?: [...{
								...
								// This is used as a hint for implementations to offer richer behavior for
								// protocols that they understand.
		...
		monitoring?: {
			...
			// Setting this to zero disables the caching mechanism and can cause heavy load
		...
		podSecurityContext?: {
			...
			// If not specified and SELinuxMount feature gate is disabled, "MountOption" is
			// used for ReadWriteOncePod volumes
		...
		postgresql?: {
			...
			extensions?: [...{
				...
				image!: {
					...
					// disk. Container creation will fail if the reference isn't present and the
					// pull fails.
			...
			// Requirements to be met by sync replicas. This will affect how the
			// "synchronous_standby_names" parameter will be
			// set up.
			syncReplicaElectionConstraint?: {
				// This flag enables the constraints for sync replicas
				enabled!: bool

				// A list of node labels values to extract and compare to evaluate if the pods
				// reside in the same topology or not
		...
		replica?: {
			...
			// Primary defines which Cluster is defined to be the primary in the distributed
			// PostgreSQL cluster, based on the
		...
		replicationSlots?: {
			...
			synchronizeReplicas?: {
				...
				// List of regular expression patterns to match the names of replication slots
				// to be excluded (by default empty)
		...
		// The time in seconds that controls the window of time reserved for the smart
		// shutdown of Postgres to complete.
	...
	status?: {
		...
		// The total number of PVC Groups detected in the cluster. It may differ from
		// the number of existing instance pods.
		instances?: int
		...
		// PGDataImageInfo contains the details of the latest image that has run on the
		...
		poolerIntegrations?: {
			// PgBouncerIntegrationStatus encapsulates the needed integration for the
			// pgbouncer poolers referencing the cluster
		...
		// The total number of ready instances in the cluster. It is equal to the number
		// of ready instance pods.
		readyInstances?: int
		...
		switchReplicaClusterStatus?: {
			// InProgress indicates if there is an ongoing procedure of switching a cluster
		...
		topology?: {
			...
			// SuccessfullyExtracted indicates if the topology data was extract. It is
			// useful to enact fallback behaviors
