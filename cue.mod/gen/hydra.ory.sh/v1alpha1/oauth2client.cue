...
#OAuth2Client: {
	...
	spec?: {
		...
		// BackChannelLogoutSessionRequired Boolean value specifying whether the RP
		// requires that a sid (session ID) Claim be included in the Logout Token to
		// identify the RP session with the OP when the backchannel_logout_uri is used.
		// If omitted, the default value is false.
		backChannelLogoutSessionRequired?: bool

		// BackChannelLogoutURI RP URL that will cause the RP to log itself out when
		// sent a Logout Token by the OP
		backChannelLogoutURI?: =~"(^$|^https?://.*)"

		// ClientName is the human-readable string name of the client to be presented to
		...
		// Values can be 'delete' to delete the OAuth2 client, value 'orphan' to keep an
		// orphan oauth2 client.
		deletionPolicy?: "delete" | "orphan"

		// FrontChannelLogoutSessionRequired Boolean value specifying whether the RP
		// requires that iss (issuer) and sid (session ID) query parameters be included
		// to identify the RP session with the OP when the frontchannel_logout_uri is
		// used
		frontChannelLogoutSessionRequired?: bool

		// FrontChannelLogoutURI RP URL that will cause the RP to log itself out when
		// rendered in an iframe by the OP. An iss (issuer) query parameter and a sid
		// (session ID) query parameter MAY be included by the OP to enable the RP to
		// validate the request and to determine which of the potentially multiple
		...
		// JwksUri Define the URL where the JSON Web Key Set should be fetched from when
		// performing the private_key_jwt client authentication method.
		jwksUri?: =~"(^$|^https?://.*)"
