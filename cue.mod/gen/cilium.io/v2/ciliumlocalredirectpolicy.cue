...
#CiliumLocalRedirectPolicy: {
	...
	spec?: {
		...
		redirectBackend!: {
			...
			// pod(s) where traffic is redirected to. When multiple ports are
			// specified, the ports must be named.
			toPorts!: [...{
				// Name is a port name, which must contain at least one [a-z], and
				// may also contain [0-9] and '-' anywhere except adjacent to
		...
		// traffic from. It can not be empty.
		redirectFrontend!: matchN(1, [{
			...
			addressMatcher?: {
				...
				// Example: When it is set to "169.254.169.254", traffic destined
				// to "169.254.169.254" is redirected.
				...
				// traffic to be redirected. When multiple ports are specified,
				// the ports must be named.
				// Example: When set to Port: "53" and Protocol: UDP, traffic
				// destined to port '53' with UDP protocol is redirected.
				toPorts!: [...{
					// Name is a port name, which must contain at least one [a-z], and
					// may also contain [0-9] and '-' anywhere except adjacent to
					// another '-' or in the beginning or the end.
			...
			serviceMatcher?: {
				// Namespace is the Kubernetes service namespace. The service
				// namespace must match the namespace of the parent Local
				...
				// identifies traffic to be redirected. The service type needs to
				// be ClusterIP.
				// Example: When this field is populated with
				// 'serviceName:myService', all the traffic destined to the
				// cluster IP of this service at the (specified) service port(s)
				// will be redirected.
				serviceName!: string

				// ToPorts is a list of destination service L4 ports with protocol
				// for traffic to be redirected. If not specified, traffic for
				// all the service ports will be redirected. When multiple ports
