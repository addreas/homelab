...
let _cue_agent_yaml = {
	...
	sinks: {
		...
		let parseableBase = {
			...
			// healthcheck: {
			// 	enabled: true
			// 	uri:     "http://parseable:8000/api/v1/liveness"
			// }
		}
		parseable_kubernetes: parseableBase & {
			inputs: ["kubernetes_logs"]
			request: headers: "X-P-Stream": "kubernetes"
		}
		parseable_journald: parseableBase & {
			inputs: ["journald_logs"]
			request: headers: "X-P-Stream": "journald"
		}
	}
}

k: ConfigMap: vector: {
	data: "agent.yaml": yaml656e63.Marshal(_cue_agent_yaml)
	metadata: labels:   vectorLabels
}

k: VMPodScrape: vector: spec: {
